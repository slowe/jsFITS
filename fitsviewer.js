...
function FitsViewer(input) {
  ...
  var html =
    'File: <select class="file" name="FITS"><option selected>WFPC2u5780205r_c0fx.fits</option><option>l_e_20110215_205_1_1_1.fits</option><option>l_e_20110215_203_1_1_1.fits</option></select>';
  html += 'Stretch function: <select class="stretch" name="stretch">';
  for (i = 0; i < this.stretches.length; i++) {
    html +=
      "<option" +
      (this.stretches[i] == this.fits.stretch ? ' selected="selected"' : "") +
      ">" +
      this.stretches[i] +
      "</option>";
  }
  ...
  for (i = 0; i < this.colors.length; i++) {
    html +=
      "<option" +
      (this.colors[i] == this.fits.color ? ' selected="selected"' : "") +
      ">" +
      this.colors[i] +
      "</option>";
  }
  ...
  if (this.capability.file) {
    ...
    $("#" + this.id).prepend(
      "<p>The File APIs are not fully supported in this browser. You are missing out on the awesome. :-(</p>"
    );
  }
...
FitsViewer.prototype.imageUnderlay = function (txt) {
  ...
  if ($("#" + this.id + " .loader").length == 0)
    $("#" + this.id).append(
      '<div class="loader"><div class="loader_inner">' + txt + "</div></div>"
    );
  else
    $("#" + this.id + " .loader")
      .html(txt)
